...
func testSlices6() {
	...
	b := append(a[0:1:2], 4) // если добавить еще один элемент, то будет уже новый базовый массив использован
	fmt.Println(cap(b))      // 2 вернет
	b[0] = 5
...
func testSlices7() {
	a := []int{1, 2, 3}
	fmt.Println(a[1:2:2], cap(a[1:2:2]), len(a[1:2:2])) //2 1 1
	b := append(a[1:2:2], 4)                            // так как cap 1, то новый базовы массив будет в b
	fmt.Println(b, cap(b), len(b))                      // 24 2 2
	b[0] = 5
	fmt.Printf("%v\n", a) //123
}
...
func testSlices8() {
	...
	//сначала ищем элемент с наименьшим индексом он будет стоять под своим индексом, все элементы до этого индекса будут заполнены базовыми значениями
	//после элемента с наименьшим индексом будут идти за ним стоящие неиндексированные элементы
	//потом ищем другой наименьший индекс ИТД
